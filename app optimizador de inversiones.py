...
def optimizar_montecarlo(retornos, n=5000):
    ...
    pesos_lista = []
    for i in range(n):
        p = np.random.random(len(tickers))
        p /= p.sum()
    ...
    return tickers, pesos_lista[idx], resultados, idx
...
# ── SELECTOR ───────────────────────────────────────────────────
st.markdown('<div class="section-title">▸ Seleccioná un sector o ingresá tus propios tickers</div>', unsafe_allow_html=True)
...
with col_custom:
    st.markdown("**Personalizá los tickers** (opcional — reemplaza la selección del sector)")
    tickers_input = st.text_input(
        "Ingresá tickers separados por coma",
    ...
    st.markdown(f"**Tickers a analizar:** `{'  |  '.join(tickers_finales)}`")
...
if correr:
    ...
    with col_g4:
        ...
        df_pesos = pd.DataFrame({'Ticker': tickers_ok, 'Peso': mejores_pesos})
        df_pesos = df_pesos.sort_values('Peso', ascending=False)
        for _, row in df_pesos.iterrows():
            pct = row['Peso'] * 100
            st.markdown(f"""
                <div class="weight-row">
                    <span style="color:{C_CYAN}; font-family:'Share Tech Mono'">{row['Ticker']}</span>
                    <span style="color:{C_TEXT}">{pct:.1f}%</span>
                </div>
